...
class Embed_Privacy {
	...
	public $embed_providers = [
		...
		'youtu.be'              => 'YouTube',
		'youtube.com'           => 'YouTube',
		'funkedigital.de'       => 'Funke Digital',
		'funkdigital.de'        => 'Funke Digital',
		'ebxcdn.com'            => 'EBX CDN',
		'adnami.io'             => 'Adnami',
		'readpeak.com'          => 'ReadPeak',
		'ravenjs.com'           => 'RavenJS',
		'sascdn.com'            => 'SAS CDN',
		'hadronid.net'          => 'HadronID',
	...
	public function clear_embed_cache() {
		...
		if ( \is_plugin_active_for_network( 'fd-og-embed-privacy/embed-privacy.php' ) ) {
	...
	public function get_embeds( $type = 'all' ) {
		...
		switch ( $type ) {
			case 'custom':
				// phpcs:disable WordPressVIPMinimum.Functions.RestrictedFunctions.get_posts_get_posts, WordPress.DB.SlowDBQuery.slow_db_query_meta_query, WordPress.DB.SlowDBQuery.slow_db_query_meta_value
	...
	private function get_local_tweet( $html ) {
		...
		return \str_replace( [ '<html><meta charset="utf-8">', '</html>' ], [
	...
	private function get_oembed_dimensions( $content ) {
		...
		foreach ( [ 'embed', 'iframe', 'img', 'object', 'blockquote', 'script' ] as $tag ) {
	...
	private function get_oembed_title( $content ) {
		...
		foreach ( [ 'embed', 'iframe', 'object', 'blockquote', 'script' ] as $tag ) {
			foreach ( $dom->getElementsByTagName( $tag ) as $element ) {
	...
	public function get_output_template( $embed_provider, $embed_provider_lowercase, $output, $args = [] ) {
		...
		if ( $embed_provider_lowercase !== 'default' ) {
            ...
            <p class="embed-privacy-input-wrapper">
                <input id="<?php echo \esc_attr( $checkbox_id ); ?>" type="checkbox" value="1"
                       class="embed-privacy-input"
                       data-embed-provider="<?php echo \esc_attr( $embed_provider_lowercase ); ?>">
                <label for="<?php echo \esc_attr( $checkbox_id ); ?>" class="embed-privacy-label"
                       data-embed-provider="<?php echo \esc_attr( $embed_provider_lowercase ); ?>">
					<?php
					/* translators: the embed provider */
		...
		?>
        <div class="embed-privacy-container is-disabled <?php echo \esc_attr( $embed_classes ); ?>"
             data-embed-id="oembed_<?php echo \esc_attr( $embed_md5 ); ?>"
             data-embed-provider="<?php echo \esc_attr( $embed_provider_lowercase ); ?>"<?php echo( ! empty( $embed_thumbnail['thumbnail_path'] ) && \file_exists( $embed_thumbnail['thumbnail_path'] ) ? ' style="background-image: url(' . \esc_url( $embed_thumbnail['thumbnail_url'] ) . ');"' : '' ); ?>>
            ...
            <style>
                <?php
				// Fallback for undefined array key 'height'
	...
	public function get_single_overlay( $content, $embed_provider, $embed_provider_lowercase, $args ) {
		...
		$args = \wp_parse_args( $args, [
			...
			'elements'            => [ 'embed', 'iframe', 'object', 'blockquote', 'script'],
			'height'              => 0,
